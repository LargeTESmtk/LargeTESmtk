...
model CoolingCurve "Verification of cooling curve according to lumped system analysis"
  ...
  LargeTESmtk.Components.GroundDomain.GridUnits.GridZone_Rect rectangGridZone(
    ...
    redeclare LargeTESmtk.Components.GroundDomain.MaterialProperties.Ground.GenericGround thermProp_unif(k=1e6),
    T_init_unif=T_init_unif) annotation (Placement(transformation(extent={{-20,10},{20,50}})));

  Modelica.Thermal.HeatTransfer.Components.Convection conBot[sum(rectangGridZone.zoneLayout_r.M_nodesPerSec)]
    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={0,-8})));
  Modelica.Blocks.Sources.RealExpression G_bot[sum(rectangGridZone.zoneLayout_r.M_nodesPerSec)](y=h*A_z_bo)
  ...
  Modelica.Thermal.HeatTransfer.Components.Convection conTop[sum(rectangGridZone.zoneLayout_r.M_nodesPerSec)]
    annotation (Placement(transformation(
        ...
        rotation=270,
        origin={0,76})));
  Modelica.Blocks.Sources.RealExpression G_top[sum(rectangGridZone.zoneLayout_r.M_nodesPerSec)](y=h*A_z_to)
    annotation (Placement(transformation(extent={{40,66},{20,86}})));
  Modelica.Thermal.HeatTransfer.Components.Convection conLeft[sum(rectangGridZone.zoneLayout_z.N_nodesPerSec)]
    annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=0,
        origin={-40,30})));
  Modelica.Blocks.Sources.RealExpression G_left[sum(rectangGridZone.zoneLayout_z.N_nodesPerSec)](y=h*A_r_le)
    annotation (Placement(transformation(extent={{-66,42},{-46,62}})));
  Modelica.Thermal.HeatTransfer.Components.Convection conRight[sum(rectangGridZone.zoneLayout_z.N_nodesPerSec)]
    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={38,30})));
  Modelica.Blocks.Sources.RealExpression G_right[sum(rectangGridZone.zoneLayout_z.N_nodesPerSec)](y=h*A_r_ri)
    annotation (Placement(transformation(extent={{66,42},{46,62}})));

//   Modelica.Thermal.HeatTransfer.Components.HeatCapacitor lumpElement(C=sum(rectangGridZone.segments[:, :].nodes[:, :].C), T(start=T_init_unif,
//         fixed=true))
//     annotation (Placement(transformation(extent={{-10,-78},{10,-98}})));

  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor lumpElement(C=sum(C_Seg[:,:]), T(start=T_init_unif,
        fixed=true))
    annotation (Placement(transformation(extent={{-10,-78},{10,-98}})));
  ...
  Modelica.Thermal.HeatTransfer.Components.Convection conLump
    annotation (Placement(transformation(
        ...
        rotation=270,
        origin={0,-62})));
  Modelica.Blocks.Sources.RealExpression G_lump(y=h*(sum(A_z_to) + sum(A_z_bo) + sum(A_r_le) + sum(A_r_ri)))
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector thermColl_to(m=sum(rectangGridZone.zoneLayout_r.M_nodesPerSec))
    annotation (Placement(transformation(
        ...
        rotation=90,
        origin={-50,80})));
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector thermColl_ri(m=sum(rectangGridZone.zoneLayout_z.N_nodesPerSec))
    annotation (Placement(transformation(
        extent={{10,10},{-10,-10}},
        rotation=270,
        origin={64,30})));
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector thermColl_le(m=sum(rectangGridZone.zoneLayout_z.N_nodesPerSec))
    annotation (Placement(transformation(
        ...
        rotation=90,
        origin={-70,30})));
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector thermColl_bo(m=sum(rectangGridZone.zoneLayout_r.M_nodesPerSec))
    annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=270,
        origin={-40,-26})));
  parameter Utilities.LayoutsAndDim.ZoneLayout_Axial_Parameters zoneLayoutPar_z(
    ...
    N_Nodes_z=10) annotation (Placement(transformation(extent={{-60,-100},{-40,-80}})));
  parameter Utilities.LayoutsAndDim.ZoneLayout_Radial_Parameters zoneLayoutPar_r(
    ...
    dr_smallestNo=0.05,
    GR_r=1.1,
    reversedDir_GR_r=true) annotation (Placement(transformation(extent={{-100,-100},{-80,-80}})));

  parameter Modelica.Units.SI.Area A_z_to[sum(rectangGridZone.zoneLayout_r.M_nodesPerSec)](each fixed=false);
  parameter Modelica.Units.SI.Area A_z_bo[sum(rectangGridZone.zoneLayout_r.M_nodesPerSec)](each fixed=false);
  parameter Modelica.Units.SI.Area A_r_le[sum(rectangGridZone.zoneLayout_z.N_nodesPerSec)](each fixed=false);
  parameter Modelica.Units.SI.Area A_r_ri[sum(rectangGridZone.zoneLayout_z.N_nodesPerSec)](each fixed=false);

  parameter Modelica.Units.SI.Temperature T_init_unif = 273.15+20 "Uniform initial temperature for all nodes of the subdomain";
  parameter Modelica.Units.SI.HeatCapacity C_Seg[rectangGridZone.zoneLayout_r.M_Sec_r,rectangGridZone.zoneLayout_z.N_Sec_z](each
      fixed=false) "Heat capacity per segment";

  Modelica.Blocks.Sources.RealExpression VC_T_GridZone(y=rectangGridZone.segments[1, 1].nodes[1, 1].T)
...
initial equation
  for m in 1:rectangGridZone.zoneLayout_r.M_Sec_r loop
    ...
    A_z_bo[(if m==1 then 1 else sum(rectangGridZone.zoneLayout_r.M_nodesPerSec[1:m-1])+1):sum(rectangGridZone.zoneLayout_r.M_nodesPerSec[1:m])]
      = rectangGridZone.segments[m,end].nodes[:,end].A_z;
  end for;

  for n in 1:rectangGridZone.zoneLayout_z.N_Sec_z loop
    A_r_le[(if n==1 then 1 else sum(rectangGridZone.zoneLayout_z.N_nodesPerSec[1:n-1])+1):sum(rectangGridZone.zoneLayout_z.N_nodesPerSec[1:n])]
      = rectangGridZone.segments[1,n].nodes[1,:].A_r_le;
    A_r_ri[(if n==1 then 1 else sum(rectangGridZone.zoneLayout_z.N_nodesPerSec[1:n-1])+1):sum(rectangGridZone.zoneLayout_z.N_nodesPerSec[1:n])]
      = rectangGridZone.segments[end,n].nodes[end,:].A_r_ri;
  end for;

  for m in 1:rectangGridZone.zoneLayout_r.M_Sec_r loop
    for n in 1:rectangGridZone.zoneLayout_z.N_Sec_z loop
      C_Seg[m,n] = sum(rectangGridZone.segments[m,n].nodes[:,:].C);
    end for;
  end for;
...
equation
  ...
  connect(conLeft.solid, rectangGridZone.heatPorts_le) annotation (Line(points={{-30,30},{-20,30}}, color={191,0,0}));
  ...
  connect(conTop.solid, rectangGridZone.heatPorts_to)
  ...
  connect(rectangGridZone.heatPorts_ri, conRight.solid) annotation (Line(points={{20,30},{28,30}}, color={191,0,0}));
  ...
  connect(conBot.solid, rectangGridZone.heatPorts_bo) annotation (Line(points={{1.77636e-15,2},{1.77636e-15,5},{0,5},{0,10}}, color={191,0,0}));
  annotation (
    experiment(
      StopTime=360000,
      Interval=600,
    ...
    Documentation(info="<html>
        ...
        <p>
        The temperatures <code>VC_T_GridZone</code> (i.e., any node temperature) and <code>VC_T_LumpElem</code> must be identical.  
        </p>
        <h4>References</h4>
        <p>
