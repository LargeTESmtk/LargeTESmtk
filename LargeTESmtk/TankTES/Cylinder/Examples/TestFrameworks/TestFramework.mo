...
model TestFramework "Test framework for example models"
  extends LargeTESmtk.TankTES.Cylinder.Examples.TestFrameworks.PartialTestFramework(break bou_T_amb);

//   replaceable Giga_TES.LargeTESmtk.TankTES.Cylinder.Examples.FullyBuried_Basic.Parameters.ModelParameters_v4_0_0_int modelPar
//     annotation (Placement(transformation(extent={{220,-80},{260,-40}})));

  Modelica.Blocks.Sources.RealExpression QFlow_loss_bot(y=storage.fluidDom.QFlow_loss_bot) annotation (Placement(transformation(extent={{244,76},{264,94}})));
  Modelica.Blocks.Continuous.Integrator Q_loss_bot
    annotation (Placement(transformation(extent={{270,78},{284,92}})));
  Modelica.Blocks.Sources.RealExpression QFlow_loss_sid(y=storage.fluidDom.QFlow_loss_sid) annotation (Placement(transformation(extent={{244,100},{264,118}})));
  Modelica.Blocks.Continuous.Integrator Q_loss_sid
    annotation (Placement(transformation(extent={{270,102},{284,116}})));
  Modelica.Blocks.Sources.RealExpression QFlow_loss_top(y=storage.fluidDom.QFlow_loss_top) annotation (Placement(transformation(extent={{244,124},{264,142}})));
  Modelica.Blocks.Continuous.Integrator Q_loss_top
    annotation (Placement(transformation(extent={{270,126},{284,140}})));
  Modelica.Blocks.Sources.RealExpression QFlow_loss_tot(y=storage.fluidDom.QFlow_loss_tot) annotation (Placement(transformation(extent={{244,52},{264,70}})));
  ...
  Modelica.Blocks.Sources.RealExpression dQint_dt_Block(y=storage.fluidDom.dQ_int_dt) annotation (Placement(transformation(extent={{176,74},{196,92}})));
  ...
  replaceable LargeTESmtk.TankTES.Cylinder.BaseClasses.PartialFullyBuried_CustomGD storage
    annotation (Placement(transformation(extent={{-60,-44},{60,76}})));
...
equation
  ...
  connect(sens_HFlow_top.port_b, storage.fluidPorts_Nod[modelPar.n_InOut_top])
    annotation (Line(points={{-120,20},{-16,20},{-16,0.4},{-15,0.4}},       color={0,127,255}));
  connect(sens_HFlow_bot.port_b, storage.fluidPorts_Nod[modelPar.n_InOut_bot])
    annotation (Line(points={{-120,-20},{-16,-20},{-16,0.4},{-15,0.4}},         color={0,127,255}));

  connect(sou_T_amb.y, storage.T_air) annotation (Line(points={{-219,100},{9.6,100},{9.6,79.6}},                   color={0,0,127}));
